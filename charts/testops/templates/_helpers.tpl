...
{{- define "renderCommonEnvs" }}
  ...
  - name: ALLURE_TASKEXECUTOR_COREPOOLSIZE
    value: "{{ printf "%.f" .Values.coreThreads }}"
  - name: ALLURE_TASKEXECUTOR_MAXPOOLSIZE
    value: "{{ printf "%.f" .Values.maxThreads }}"
...
{{- $s3Prefix := ternary "S3SHARDED" "S3" (eq .Values.storage.type "S3_SHARDED") }}
  ...
  - name: ALLURE_BLOBSTORAGE_MAXCONCURRENCY
    value: "{{ printf "%.f" .Values.maxS3Connections }}"
  - name: ALLURE_BLOBSTORAGE_TASKEXECUTOR_COREPOOLSIZE
    value: "{{ printf "%.f" .Values.coreS3Threads }}"
  - name: ALLURE_BLOBSTORAGE_TASKEXECUTOR_MAXPOOLSIZE
    value: "{{ printf "%.f" .Values.maxS3Threads }}"
  - name: ALLURE_BLOBSTORAGE_TASKEXECUTOR_QUEUECAPACITY
    value: "{{ printf "%.f" .Values.threadPoolS3QueueSize }}"
